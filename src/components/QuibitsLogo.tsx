...
export default function QuibitsLogo({
  ...
  return (
    <button
      ...
      className={`flex items-center space-x-3 transition-all duration-300 transform hover:scale-105 ${onClick ? 'cursor-pointer' : 'cursor-default'
        }`}
    >
      {/* Logo Container */}
      <div className={`relative ${sizeClasses[size].split(' ')[0]} ${sizeClasses[size].split(' ')[1]} bg-gradient-to-br from-purple-500 via-indigo-600 to-blue-600 rounded-xl flex items-center justify-center shadow-lg overflow-hidden group`}>
        {/* Animated Background */}
        <div className={`absolute inset-0 bg-gradient-to-br from-cyan-400 to-purple-600 transition-opacity duration-300 ${isHovered ? 'opacity-100' : 'opacity-0'
          }`}></div>
        ...
        {/* Glow Effect */}
        <div className={`absolute inset-0 rounded-xl transition-all duration-300 ${isHovered
      ...
      {showText && (
        <div className="flex flex-col">
          <span className={`font-bold transition-colors duration-300 ${size === 'sm' ? 'text-lg' : size === 'md' ? 'text-xl' : 'text-2xl'
            } ${isDarkTheme ? 'text-white' : 'text-gray-900'}`}>
            SmartDash AI
          </span>
          <span className={`text-xs transition-colors duration-300 ${isDarkTheme ? 'text-purple-400' : 'text-purple-600'
            }`}>
            AI Intelligence
          </span>
        </div>
