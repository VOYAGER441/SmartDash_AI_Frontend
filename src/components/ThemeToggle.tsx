...
export default function ThemeToggle({ isDarkTheme, onToggle, size = 'md', variant = 'button' }: ThemeToggleProps) {
  ...
  if (variant === 'switch') {
    return (
      <button
        ...
        className={`relative inline-flex items-center justify-center p-1 rounded-full transition-all duration-300 ${
          isDarkTheme 
        ...
        <div className={`relative w-16 h-8 rounded-full ${isDarkTheme ? 'bg-purple-900/50' : 'bg-gray-200'} border ${isDarkTheme ? 'border-purple-400/30' : 'border-gray-300'}`}>
          ...
          >
            ...
            <div className={`absolute inset-0 rounded-full animate-ping ${
              isDarkTheme ? 'bg-yellow-400/50' : 'bg-blue-600/50'
            }`}></div>
          </div>
        </div>
  ...
  if (variant === 'floating') {
    return (
      <button
        ...
        onMouseEnter={() => setIsHovered(true)}
        onMouseLeave={() => setIsHovered(false)}
        className={`fixed bottom-8 right-8 group p-4 rounded-full transition-all duration-300 transform hover:scale-110 ${
          isDarkTheme 
      ...
      >
        ...
        <div className="relative">
          ...
          <div className="absolute inset-0">
            <div className={`absolute top-0 left-1/2 w-2 h-2 rounded-full animate-spin ${
              isDarkTheme ? 'bg-yellow-400' : 'bg-blue-400'
            }`} style={{ animationDuration: '3s' }}></div>
            <div className={`absolute bottom-0 left-1/2 w-2 h-2 rounded-full animate-spin ${
              isDarkTheme ? 'bg-orange-400' : 'bg-purple-400'
            }`} style={{ animationDuration: '2s', animationDirection: 'reverse' }}></div>
            <div className={`absolute top-1/2 left-0 w-2 h-2 rounded-full animate-spin ${
              isDarkTheme ? 'bg-yellow-300' : 'bg-indigo-400'
            }`} style={{ animationDuration: '4s' }}></div>
          </div>
        </div>

        {/* Tooltip */}
        <div className={`absolute bottom-full mb-2 px-3 py-1 rounded-lg text-xs font-medium whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity duration-300 ${
          isDarkTheme ? 'bg-gray-800 text-white' : 'bg-gray-800 text-white'
        }`}>
  ...
  return (
    ...
    >
      ...
      <div className="relative">
        <div className={`${sizeClasses[size]} rounded-full flex items-center justify-center transition-all duration-500 transform group-hover:rotate-180 group-hover:scale-110 ${
          isDarkTheme 
            ? 'bg-gradient-to-br from-yellow-400 to-orange-500' 
            : 'bg-gradient-to-br from-blue-600 to-purple-600'
        } shadow-lg`}>
        ...
        {/* Animated glow ring */}
        <div className={`absolute inset-0 rounded-full animate-ping ${
          isDarkTheme ? 'bg-yellow-400/30' : 'bg-blue-600/30'
        }`}></div>
        
        {/* Pulsing dot */}
        <div className={`absolute -top-1 -right-1 w-3 h-3 rounded-full animate-pulse ${
          isDarkTheme ? 'bg-yellow-400' : 'bg-blue-600'
        }`}></div>
      </div>
    </button>
  )
